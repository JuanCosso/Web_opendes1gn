...
export default function CategoriasAdminPage() {
  ...
  async function handleDelete(cat: Category) {
    ...
    const data = await res.json();
    if (!res.ok) { setError(data.error || "No se pudo eliminar."); setDeletingId(null); return; }
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-12">
        ...
        <Link href="/admin/categorias/nueva" className="btn-primary w-full sm:w-auto text-center">
          + Nueva categoría
        </Link>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map((cat) => (
            <div key={cat.id}
              ...
              <div className="relative h-36 bg-[var(--blush)] overflow-hidden">
                {cat.image ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <span className="text-5xl font-light text-[var(--text-light)]/20 italic select-none"
                      style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                      {cat.name.charAt(0)}
                    </span>
                  </div>
              ...
              <div className="p-5">
                <h3 className="text-[16px] font-light text-[var(--text)] mb-1"
                  style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                  {cat.name}
                </h3>
                <p className="text-[11px] text-[var(--text-light)] tracking-[1px] mb-1">/{cat.slug}</p>
                {cat.description && (
                  <p className="text-[12px] text-[var(--text-light)] font-light mt-2 line-clamp-2">{cat.description}</p>
                )}
