    ...
    <div className="max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className={card}>
          <h2 className="text-[12px] tracking-[3px] uppercase text-[var(--accent)]">
            Información básica
          </h2>
          <div>
          ...
          <div className="flex gap-6 pt-2">
            <label className="flex items-center gap-3 cursor-pointer group">
              <input
                name="published"
                type="checkbox"
                className="w-4 h-4 border border-[var(--border)] rounded-[var(--radius-sm)] accent-[var(--accent)]"
              />
        ...
        <section className={card}>
          ...
          <p className="text-[12px] text-[var(--text-light)] font-light -mt-2">
            Talle, color y stock por variante
          </p>
          <div className="space-y-5">
            {variants.map((v, i) => (
              <div
                key={i}
                className="grid grid-cols-[1fr_1fr_80px_auto] gap-4 items-end pb-5 border-b border-[var(--border)] last:border-0 last:pb-0"
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setVariants((vs) =>
                        vs.map((vv, j) => (j === i ? { ...vv, size: e.target.value } : vv))
                      )
                    }
                    className={input}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setVariants((vs) =>
                        vs.map((vv, j) => (j === i ? { ...vv, color: e.target.value } : vv))
                      )
                    }
                    className={input}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setVariants((vs) =>
                        vs.map((vv, j) =>
                          j === i ? { ...vv, stock: parseInt(e.target.value) || 0 } : vv
                        )
                      )
                    }
                    className={input}
                ...
                {variants.length > 1 && (
                  <button
                    ...
                    className="pb-2.5 text-[var(--text-light)] hover:text-red-400 transition-colors text-xl leading-none"
                  >
                    ×
                  </button>
                )}
          ...
          <button
            type="button"
            onClick={() =>
              setVariants((v) => [...v, { size: "", color: "", stock: 0, price: "", sku: "" }])
            }
        ...
        <div className="flex gap-3 pt-2 pb-6">
          <button
            ...
            className="px-8 py-3.5 bg-[var(--accent)] text-[var(--cream)] text-[11px] tracking-[0.18em] uppercase hover:opacity-90 transition-opacity disabled:opacity-50 rounded-[var(--radius-sm)]"
          >
          ...
          <button
            type="button"
            onClick={() => router.back()}
            className="px-8 py-3.5 border border-[var(--border)] text-[var(--text-light)] text-[11px] tracking-[0.18em] uppercase hover:bg-[var(--blush)] transition-colors rounded-[var(--radius-sm)]"
          >
            Cancelar
          </button>
        </div>
